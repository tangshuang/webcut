...
const touchVideoVolumeHistory = () => touchHistory({ title: '调整视频音量', mergeKey: `video:${currentSource.value?.key}:volume` });
const touchPlaybackRateHistory = () => touchHistory({ title: '调整播放速度', mergeKey: `video:${currentSource.value?.key}:rate` });
...
function syncVolumeToForm() {
  ...
  const videoVolume = currentSource.value.meta.video?.volume;
  volume.value = videoVolume ?? 1;
...
watch(volume, (newVolume) => {
  ...
  syncSourceMeta(currentSource.value, {
    video: { ...currentSource.value.meta.video, volume: newVolume }
  });
