...
export const applyAnimation: WebCutAgentTool<{ sourceKey: string; type: 'enter' | 'exit' | 'motion'; name: string; durationUs?: number; delayUs?: number; iterCount?: number }> = {
    name: 'webcut_apply_animation',
    description: '给片段应用动画。type=enter(入场)/exit(出场)/motion(循环运动)；name 取自 webcut_list_effects 的 animations（如 fadeIn/slideInLeft/pulse/shake）。durationUs/delayUs 单位微秒；iterCount 0=无限（仅 motion）。' + REFRESH_HINT,
    parameters: {
        type: 'object',
        properties: {
...
export const applyTransition: WebCutAgentTool<{ railId: string; name: string; durationUs?: number }> = {
    name: 'webcut_apply_transition',
    description: '在指定轨道上、当前播放头所在的两个相邻片段之间添加转场。name 取自 webcut_list_effects 的 transitions（fade/slide/zoom/blinds/dissolve）。先调 webcut_seek_cursor 把播放头移到两段衔接处。' + REFRESH_HINT,
    parameters: {
        type: 'object',
        properties: {
